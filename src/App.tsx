        ...
        <DialogContent className="max-w-7xl max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="flex items-center gap-2 mb-4">
            <div className="relative flex-1">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
              <Input placeholder="Search..." value={dialogSearchTerm} onChange={e => setDialogSearchTerm(e.target.value)} className="pl-9" />
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 sticky top-0">
                  <tr>
                    {columns.map(col => (
                      <th key={col} onClick={() => handleDialogSort(col)} className="px-4 py-3 text-left font-medium text-slate-700 border-b border-slate-200 cursor-pointer hover:bg-slate-100 transition-colors whitespace-nowrap">
                ...
                <tbody>
                  {getDialogFilteredData().map((row, idx) => (
                    <tr key={idx} className="hover:bg-slate-50 transition-colors">
                      {columns.map(col => (
                        <td key={col} className="px-4 py-2 border-b border-slate-100 whitespace-nowrap max-w-xs overflow-hidden text-ellipsis">
                          {row[col] != null ? String(row[col]) : '-'}
                        </td>
                      ))}
                    </tr>
